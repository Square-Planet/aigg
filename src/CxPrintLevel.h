...
struct TPrintOptions : public FPrintOptionsBase {
   ...
   FPrintLevel operator [](unsigned iOption) const { return FPrintLevel(this->at(iOption)); }

   void SetArgs(std::string const &Args, FGetOptionIdFn pOptionId);
   void SetArgs(string_slice const &Args, FGetOptionIdFn pOptionId);
   void Set(unsigned iOption, FPrintLevel const &pl) { _Set1(iOption, pl, &m_OptionValues[0], this->size()); };
   void Set(unsigned iOption, string_slice const &slValue) { _Set1(iOption, slValue, &m_OptionValues[0], this->size()); };
   void SetAll(FPrintLevel pl) { this->fill(FStoredType(int(pl))); }
...
//    FLevel operator [](unsigned iOption) const { return m_OptionValues.at(iOption); }
//    bool SupressQ(unsigned iOption) const { return m_OptionValues.at(iOption) < 0; }
//    bool OffQ(unsigned iOption) const { return m_OptionValues.at(iOption) <= 0; }
//    bool BasicQ(unsigned iOption) const { return m_OptionValues.at(iOption) >= 1; }
//    bool MoreQ(unsigned iOption) const { return m_OptionValues.at(iOption) >= 2; }
//    bool MostQ(unsigned iOption) const { return m_OptionValues.at(iOption) >= 3; }
//    bool FullQ(unsigned iOption) const { return m_OptionValues.at(iOption) >= 4; }
#endif // INCLUDE_ABANDONED
protected:
   FStoredType
      m_OptionValues[NumOptionsT];
   std::size_t size() const { return NumOptionsT; }
   void fill(FStoredType v) {
      for (std::size_t i = 0; i != this->size(); ++ i)
         m_OptionValues[i] = v;
   }
   FStoredType at(std::size_t i) const {
      if (i >= size()) throw std::out_of_range("TPrintOptions<N>: invalid option access");
      return m_OptionValues[i];
   }
};
